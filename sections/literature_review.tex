...
classical paths for a VQE-type circuit, however, it is limited by the VRAM of a single accelerator. This creates a "memory wall" 
where the simulation of molecules larger than $LiH$ (4 electrons, 631 Pauli terms, requiring 12 qubits)\cite{kandala2017hardware} is unobtainable on a single workstation. 
This stack follows their findings and we adopt a multi-node distributed memory model using MPI to orchestrate the GPU-equipped A100 nodes. Distributing the Pauli string expectation value 
estimations across a GPU cluster allows the stack to aggregate the VRAM of the entire cluster to allow for larger chemical systems to be simulated 
and/or optimized (such as $BeH_2$ (6 electrons, 666 Pauli terms, requiring 14 qubits))\cite{kandala2017hardware} without exceeding the capacity of a single NVIDIA A100 GPU. 
...
while overlooking the potential computation speedup offered by parallelism of parameters (the portability of workloads). VQE is a SIMD problem, 
where the same circuit is executed with thousands of parameter variations in order to find the wavefunction ground state. 
By considering the SIMD-centric findings of Bayraktar et al. \cite{bayraktar2023cuquantum}, this stack incorporates Parameter Batching on the 
dispatcher level. Instead of a synchronous serial loop of submitting, waiting, and receiving, the middleware bundles variational points 
into a single job payload. This reduces the overhead from the quantum control stack and exploits the GPU ability to handle 
the high-throughput numerical gradients in parallel. 
