...
(ranging between 12--9,000 for the simulator path) confirm that stack's classical computation is the primary contributor to communication 
...
disribution on the shared memory alone is not useful, GPU acceleration is the defining cause of a MPI distributed speedup.
...
% The experiments results validates several of the predictions made in this paper's literature review. \textcite{pellow2021classicoptimizers}
...
% ansatz, which was identified by \textcite{mccaskey2019quantumchemistrybenchmarknearterm} regarding the 
% preparation of states with a varying range of electron numbers, was also observed in the particle number violations 
% (discussed in Section~\ref{sec:hwe_limitation}). \textcite{bayraktar2023cuquantum} confirm the GPU acceleration improvement in the molecule $H_{2}O$ when scaling from $0.24\times$ to $1.51\times$ at $P=8$,
% directly supporting their case for GPU-accelerated statevector construction for larger workloads. Finally, $M \gg 1$ 
