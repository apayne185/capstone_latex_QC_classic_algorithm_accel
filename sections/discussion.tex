...
It should be stated that this is a previously known limitation of HWE ansatzes, as documented by Kandala et al.~\parencite{kandala2017hardware} 
...
or Probabilistic Error Cancellation(PEC) would be needed so that hardware noise can be suppressed, as discussed by Cai et al.~\parencite{cai2023quantumerrormitigation}.
...
Amdahl's Law~\parencite{amdahl1967validity} where the maximum speedup will be bounded by $1/f$ despite of the number of processors utilized. 
...
a $6\times$ improvement. This result confirms the statevector construction bottleneck prediction from Bayraktar et al.~\parencite{bayraktar2023cuquantum}.
...
The experiments results validates several of the predictions made in this paper's literature review. Pellow-Jarman et al.~\parencite{pellow2021classicoptimizers} 
...
ansatz, which was identified by McCaskey et al.~\parencite{mccaskey2019quantumchemistrybenchmarknearterm} regarding the 
...
directly supporting their case for GPU-accelerated statevector construction for larger workloads \parencite{bayraktar2023cuquantum}. Finally, $M \gg 1$ 
across all execution paths demonstrate that classical computation still dominates communication overhead, not the Communication Wall \parencite{weder2021quantum}.
