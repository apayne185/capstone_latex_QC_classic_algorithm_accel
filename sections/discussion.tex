...
It should be stated that this is a previously known limitation of HWE ansatzes, as documented by Kandala et al.~\cite{kandala2017hardware} 
...
trajectories due to the usages of different execution paths, the distributed \texttt{Allreduce} sum is error 
...
(ranging between 12--9,000 for the simulator path) confirm that stacks classical computation is the primary contributor to communication 
...
Amdahl's Law~\cite{amdahl1967validity} where the maximum speedup will be bounded by $1/f$ despite of the number of processors utilized. 
...
a $6\times$ improvement. This result confirms the statevector construction bottleneck prediction from Bayraktar et al.~\cite{bayraktar2023cuquantum}.

A notable finding is that the cuStateVec backend performed slower than GPU Aer ($242$,s vs. $161$,s for $H_{2}O$ at $P=4$), 
resulting due to GTX 1650 Mobile's limited memory bandwidth ($\sim$128 GB/s GDDR6), which insufficient for the HBM bandwidth that cuStateVec was designed for, as discussed in Section~\ref{sec:methodology}. 

\subsection{Stack Relationship to XACC}
This stack builds on XACC's philosophy~\cite{mccaskey2018xacc} of handling the QPU as an accelerated coprocessor with a hardware-agnostic and language-agnostic
...
physical QPUs), and this was reconfirmed within both of the stack's simulator and IBM QPU paths. SPSA converged reliably across 
...
(discussed in Section~\ref{sec:hwe_limitation}). Bayraktar et al. confirm the GPU acceleration improvement in the molecule $H_{2}O$ when scaling from $0.24\times$ to $1.51\times$ at $P=8$, 
directly supporting their case for GPU-accelerated statevector construction for larger workloads \cite{bayraktar2023cuquantum}. Finally, $M \gg 1$ 
across all execution paths demonstrate that classical computation still dominates communication overhead, not the Communication Wall \cite{weder2021quantum}.
