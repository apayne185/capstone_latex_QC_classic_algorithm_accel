...
and a comparable error for $BeH_2$ (39.5 vs. 39.4 mHa). This indicates the integration of MPI distribution does not necessarily improve the stack's accuracy, as both tested versions of the 
...
bottleneck, so the Quantum 'Communication Wall' ~\parencite{weder2021quantum} is not the main limiting factor of the stack's performance when compute dominates.
...
This result is explained by the hardware mismatch: Bayraktar et al.~\parencite{bayraktar2023cuquantum} established 
...
Resource Utilization & $U_{gpu}$ & $\geq 80\%$ occupancy & GPU+QPU integrated (GTX 1650 + \texttt{ibm\_marrakesh}); $6\times$ weak scaling at $P$=8 & Partial (consumer GPU) \\ \hline
