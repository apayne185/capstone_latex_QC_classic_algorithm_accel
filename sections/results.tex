...
\centering
\renewcommand{\arraystretch}{1.4}
\caption{Serial baseline VQE results (single process, no MPI, identical SPSA hyperparameters, seed=42). Both the distributed stack and serial baseline use near zero parameter initialization, track the best-physical energy.}
...
of 1.6 mHa (1 kcal/mol). Notebly from these results, the serial baseline achieved a lower error on $LiH$ (2.2 mHa compared to the much higher 336.2 mHa) 
and also with the error of $BeH_2$. This indicates to us the integration of MPI distribution does not necessarily improve the stack's accuracy, as both tested versions of the 
stack use idential near zero initialization and the exact same SPSA hyperparameters. This means the accuracy differences are stemming from 
the random divergence in SPSA tragectories within different execution tasks (the distributed \texttt{Allreduce}  vs serial summation) 
instead of from an algorithmic advantage from VQE. The molecule $BeH_{2}$ obtained 39.4 mHa within this set deterministic run, although in a separate 
trial (when using a different random seed) achieved 0.8 mHa, within chemical accuracy. This finding demonstrates that the stack is capable of reaching the accuracy target in the conditions that the SPSA trajectory is favorable.
Based on the results of the wall-clock comparison (Table~\ref{table:speedup}), we can conclude that the MPI distributed stack achieves a meaningful speedup, 
...
\centering
\renewcommand{\arraystretch}{1.4}
\caption{Wall-clock comparison between the serial baseline and distributed stack at $P=2$ MPI ranks. The speedup is calculated by $T_{\text{serial}} / T_{\text{distributed}}$. This distributed stack achieved meaningful speedup, only for molecules that have a sufficently large number of distributable Pauli terms.}
\label{table:speedup}
...
IBM QPU & $<1$\textsuperscript{$\dagger$} & QPU RTT (32-60\,s) dominates, no concurrent classical overlap in the current build \\ \hline
\end{tabular}
\end{table}

All the tested molecules achieved $M \gg 1$, confirming to us that both the statevector construction and Pauli term evaluation 
(the acceleration workload) completely mask the MPI communication overhead. This result validates this projects core 
architectural claim, that distributing a classical subroutine across several MPI ranks will not introduce a communication 
bottleneck, so the Quantum 'Communication Wall' ~\cite{weder2021quantum} is not the main limiting factor of the stacks performance when compute dominates.
...
\begin{minipage}{\textwidth}
\centering
\caption{Strong scaling efficiency for molecule $H_{2}O$ (1086 Pauli terms, 896 iterations). Efficiency $E = T_1/ (P \times T_P)$, $P=2$ result demonstrates meaningful parallelism. The degradation occuring at $P \geq 4$ is due to the shared memory contention occuring on a single Docker host.}
...
The scaling behavior is expected to see significant improvements when handling larger, more complext molecules, ones that have more Pauli 
terms to be distributed ($H_{2}O$ = 1086 terms vs. $LiH$ = 631 terms). Future tests will include molecules that will produce more computationally dense workloads. 
This, paired with the utilization of GPU accelerated statevector construction with cuQuantum, would transfer the stacks computational bottleneck from the per rank statevector 
builds to its parallelized Pauli terms evaluation.
...
The total wall-clock time grows with problem size, which is expected because in weak scaling analysis, it simultaneously increases both 
the Hamiltonian complexity and the qubit count (4$\rightarrow$12$\rightarrow$14 qubits), meaning that the $O(2^n)$ 
...
iteration increaseing from $25\;\mu$s ($P=1$) to $256$\,ms ($P=8$), however, the per term communication cost actually decreases 
from $\sim$17\,$\mu$s/term to $\sim$2.4\,$\mu$s/term. This is confirming that \texttt{MPI\_Allreduce} scales efficiently.
...
path and T-REx with a resilience level 1 only handles quantum measurement readout errors, but does not addressing gate errors or decoherence 
...
that the cloud QPU latency overhead that was predicted in Methodology, with the QPU RTT exceeding the classical computation time by 
...
the GPU accelerated stack does achieves meaningful speedup at $P=4$ and continues to improve at $P=8$ instead of providing diminishing returns.
...
GPU acceleration of gate operations will reduces this cost sufficiently for MPI Pauli term distribution to 
...
This result is explained by the hardware mismatch: Bayraktar et al.~\cite{bayraktar2023cuquantum} established 
...
The $6\times$ improvement at $P=8$ ($H_{2}O$) confirms to us that GPU acceleration effectively reduces the $O(2^n)$ 
statevector bottleneck that dominated the previous CPU only weak scaling results.

It should be noted for the smaller molecules at low rank counts, the GPU path is slower than CPU. LiH at $P=2$ takes $0.719$\,s 
on GPU vs. $0.576$\,s on CPU. This reflects the GPU kernel launch and data transfer overhead on the GTX 1650 Mobile exceeds the 
acceleration benefit for circuits that have 12 or less qubits. The GPU advantage is only observable at $P \geq 4$ 
with workloads of larger molecules such as $BeH_{2}$ and $H_{2}O$, where the $O(2^n)$ statevector cost is 
large enough for GPU acceleration to outweigh its overhead.

\subsection{Checkpoint Resilience}
The implemented 7 layer diagnostic test has verified the checkpoint resilience across all runs. After 5 iterations for 
$H_{2}$, the optimizer state ($\theta \in \mathbb{R}^{24}$) was then saved to \texttt{checkpoint\_iter\_0005.npy}. 
AT the stack's tested restart, the VQE correctly resumed from iteration 6, using the saved $\theta$ and the energy 
trajectory continued on without error. This stack's checkpoint system does rotate the saved files to keep 
only the 5 most recent saves (preventing unbounded disk usage during any long optimization runs).
